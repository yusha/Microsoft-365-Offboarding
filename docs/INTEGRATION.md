...
2. Add these **application** Microsoft Graph permissions and grant admin consent:
   ...
   `User-PasswordProfile.ReadWrite.All` is required for the Step 1 password reset, and `RoleManagement.ReadWrite.Directory` for the pre-step that removes a departing administrator's directory roles (a privileged account cannot be disabled until its roles are removed). To offboard users who hold **privileged** roles unattended, also assign the app's service principal the **Privileged Role Administrator** role. Interactive runs must be performed by a **Global Administrator**.
